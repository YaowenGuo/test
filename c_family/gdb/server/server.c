//a server to remotely run the w commend
//user can check load on moacine without login privileges
//usage: svr
...
int clntdesc, //socket descriptor for individual client
    svrdesc;        //general cocket descriptor for server
...
void respond(){
    ...
    unlink( "tmp.client" );     //remove to the file
    close( clntdesc );
}
...
int main(){
    ...
    //OK, listen in loop for clinet calls.
    listen( svrdesc, 5 );
